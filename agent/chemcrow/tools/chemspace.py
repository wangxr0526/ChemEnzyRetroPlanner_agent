...
class ChemPriceAPI:
    def __init__(self, molport_api_key) -> None:
        self.molport_api_key = molport_api_key 
        
    
    def by_mol(self, smiles):
        priceCollector = PriceCollector()
        priceCollector.setMolportApiKey(self.molport_api_key)
        priceCollector.check()
        ...
        # purchasable = purchasable_check(smiles)
        all_prices = priceCollector.collect([smiles])
        try:
            if len(all_prices) == 0:
                return "No purchasing information found for the compound."

        except KeyError:
            return "Invalid query, try something else. "
        
        
        best_price = priceCollector.selectBest(all_prices)
        return f"{best_price['Amount'][0]} {best_price['Measure'][0]} {best_price['Purity'][0]} of this molecule cost {best_price['Price_USD'][0]} USD and can be purchased at {best_price['Supplier Name'][0]}."


class GetMoleculePrice(BaseTool):
    name = "GetMoleculePrice"
    description = "Get the cheapest available price of a molecule, the input should be a molecule smiles."
    chemspace_api_key: str = None
    molport_api_key: str = None
    url: str = None
    chemprice: ChemPriceAPI = None

    def __init__(self, chemspace_api_key: str = None, molport_api_key: str = None):
        super().__init__()
        self.chemspace_api_key = chemspace_api_key
        self.molport_api_key = molport_api_key
        ...
        if molport_api_key:
            self.chemprice = ChemPriceAPI(self.molport_api_key)

    def _run(self, query: str) -> str:
        if (not self.chemspace_api_key) and (not self.molport_api_key):
            return "No Price API key found. This tool may not be used without a Price API key (support Molport and  ChemSpace)."
        elif self.chemspace_api_key:
            try:
                ...
                return str(e)
        elif self.molport_api_key:
            try:
                price = self.chemprice.by_mol(query)
                return price
            except Exception as e:
                return str(e)
