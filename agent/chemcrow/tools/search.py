...
def scholar2result_llm_ollama(llm, query, k=5, max_sources=2, semantic_scholar_api_key=None):
    """Useful to answer questions that require
    technical knowledge. Ask a specific question."""
    papers = paper_search(llm, query, semantic_scholar_api_key=semantic_scholar_api_key)
    if len(papers) == 0:
    ...
    settings = paperqa.Settings(
        ...
        summary_llm=f"ollama/{llm.model}",
...
class Scholar2ResultLLM(BaseTool):
    ...
    def __init__(self, llm, semantic_scholar_api_key, openai_api_key=None):
        super().__init__()
        self.llm = llm
        # api keys
    ...
    def _run(self, query) -> str:
        if self.openai_api_key:
            ...
            return scholar2result_llm_ollama(
                self.llm,
                query,
                semantic_scholar_api_key=self.semantic_scholar_api_key
            )
